...
const lessonsSection = html.indexOf('data-sidebar-section="lessons"');
const verbsSection = html.indexOf('data-sidebar-section="verbs"');
const exercisesSection = html.indexOf('data-sidebar-section="exercises"');
assert.ok(lessonsSection < verbsSection && verbsSection < exercisesSection, 'Verbos deve aparecer entre Aulas e Exercícios');
assert.match(html, /data-route="verbs-starter"/);
assert.match(html, /data-i18n="sidebar\.verbs"/);
assert.ok(scriptSources.findIndex((source) => source.startsWith('js/verbs.js')) < scriptSources.findIndex((source) => source.startsWith('js/app.js')));
